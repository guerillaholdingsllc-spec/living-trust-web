...
export default function LivingTrustFramework() {
  ...
  // Auth-only boot: get current user; skip getMyTrusts (not implemented yet)
  useEffect(() => {
    ...
    getCurrentAccount()
      .then(({ user: account }) => {
        ...
        // Temporary: trusts not loaded until backend is added
        setAccountTrusts([]);
      })
      .catch(() => {
  ...
  async function submitAuth(event) {
    ...
    try {
      ...
      // trusts will load later when backend features are added
      setAccountTrusts([]);
  ...
  // --- Temporary stubs for non-auth actions (to avoid build/runtime errors) ---
  async function submitPackage(event) {
    event.preventDefault();
    setStatus({
      state: "error",
      message: "Document generation is not enabled yet. Connect a backend or Supabase Edge Function."
    });
  }
